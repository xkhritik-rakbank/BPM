...
public StringBuilder getDynamicHtml_second(String inputXML, Map map, String Workstep,String CategoryID,String SubCategoryID, String outputData3, String DispColCount,String temp2[][],String sMappOutPutXML,com.newgen.wfdesktop.session.WDSession,String sCabName ,String sSessionId , string sJtsIp ,int iJtsPort)
{	
	try{
		...
		{	if(temp2[j][1].indexOf("~")>-1)
			{
				...
				inputData2 = "<?xml version='1.0'?><APSelectWithColumnNames_Input><Option>APSelectWithColumnNames</Option><Query>" + QR + "</Query><EngineName>" + sCabname + "</EngineName><SessionId>" + sSessionId + "</SessionId></APSelectWithColumnNames_Input>";*/
				...
				//outputData2 = WFCallBroker.execute(inputData2, wfsession.getJtsIp(), wfsession.getJtsPort(), 1);
				  ...
				  outputData2 = NGEjbClient.getSharedInstance().makeCall(ipAddr, String.valueOf(iJtsPort),"WebSphere" , inputData2);
				...
				if(mainCodeData3.equals("0"))
				{
					...
					{	FormValue=FormValue.substring(FormValue.indexOf("$")+1);
						FormValue=getSessionValues(FormValue, wDsession);
...
String input1 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("repeatValue"), 1000, true) );
			String repeatValue_Esapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1!=null?input1:"");
			WriteLog("Integration jsp: repeatValue_Esapi: "+repeatValue_Esapi);
			...
			String input3 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("Frameorder"), 1000, true) );
			String Frameorder_Esapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input3!=null?input3:"");
			WriteLog("Integration jsp: Frameorder_Esapi: "+Frameorder_Esapi);
			...
			String CategoryID_Esapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4!=null?input4:"");
			WriteLog("Integration jsp: CategoryID_Esapi: "+CategoryID_Esapi);
			...
			String SubCategoryID_Esapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5!=null?input5:"");
			WriteLog("Integration jsp: SubCategoryID_Esapi "+SubCategoryID_Esapi);
			
			String input6 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("panNo"), 1000, true) );
			String panNo_Esapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input6!=null?input6:"");
			WriteLog("Integration jsp: panNo_Esapi "+panNo_Esapi);
			
			String input7 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("repeatValue"), 1000, true) );
			String repeatValue_Esapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input7!=null?input7:"");
			WriteLog("Integration jsp: repeatValue_Esapi "+repeatValue_Esapi);
...
int repeatValue=Integer.parseInt(repeatValue_Esapi);
String Frameorder = Frameorder_Esapi;
String CategoryID = CategoryID_Esapi;
String SubCategoryID = SubCategoryID_Esapi;
String WS =WSNAME_Esapi;
...
String QueryProcessName = "select FrmLout.Frameorder,FrmLout.WS_NAME Logical_Name,FrmLout.LabelName, FrmLout.FieldType,FrmLout.FieldLength,FrmLout.ColumnName,FrmLout.FieldOrder, rsmap.MW_RESPMAP, FrmLout.SubCatIndex, FrmLout.IsEditable, FrmLout.IsMandatory, FrmLout.Pattern, FrmLout.CatIndex, FrmLout.event_function,FrmLout.is_workstep_req, svc.Transaction_Table, dynvar.FieldValue, wrkstp.Workstep_Name WS_Name  from USR_0_SRM_FORMLAYOUT FrmLout, USR_0_SRM_CATEGORY cat, USR_0_SRM_SUBCATEGORY subcat, USR_0_SRM_INT_RSMAP rsmap,  USR_0_SRM_SERVICE svc,  USR_0_SRM_WORKSTEPS wrkstp, usr_0_srm_dynamic_variable_master dynvar where cat.CategoryIndex=FrmLout.CatIndex  and subcat.SubCategoryIndex=FrmLout.SubCatIndex  and cat.CategoryIndex=subcat.ParentCategoryIndex and upper(rsmap.FIELDNAME)='FETCH'  and   cat.CategoryIndex=rsmap.CatIndex  and subcat.SubCategoryIndex=rsmap.SubCatIndex  and cat.CategoryName ='"+CategoryID.toUpperCase()+"'  AND   subcat.SubCategoryName='"+SubCategoryID.toUpperCase()+"'  and svc.SubCatIndex=subcat.SubCategoryIndex and FrmLout.IsActive = 'Y' and wrkstp.SR_ID=svc.SR_ID and svc.CatIndex=cat.CategoryIndex and svc.SubCatIndex=subcat.SubCategoryIndex and wrkstp.logical_name = FrmLout.ws_name and wrkstp.Workstep_NAME='"+WS.toUpperCase()+"' and cat.CategoryIndex=dynvar.CatIndex  and FrmLout.Frameorder="+Frameorder+" and subcat.SubCategoryIndex=dynvar.SubCatIndex and dynvar.FieldName='DISP_COL_COUNT' order by FrmLout.Frameorder,FrmLout.FieldOrder";
	...
	inputData= "<?xml version='1.0'?><APSelectWithColumnNames_Input><Option>APSelectWithColumnNames</Option><Query>" + QueryProcessName + "</Query><EngineName>" + wDSession.getM_objCabinetInfo().getM_strCabinetName() + "</EngineName><SessionId>" + wDSession.getM_objUserInfo().getM_strSessionId() + "</SessionId></APSelectWithColumnNames_Input>";*/
...
"</APSelectWithNamedParam_Input>";
	...
	inputData2 = "<?xml version='1.0'?><APSelectWithColumnNames_Input><Option>APSelectWithColumnNames</Option><Query>" + qry + "</Query><EngineName>" + wDSession.getM_objCabinetInfo().getM_strCabinetName() + "</EngineName><SessionId>" + wDSession.getM_objUserInfo().getM_strSessionId() + "</SessionId></APSelectWithColumnNames_Input>";*/
			...
			{	if(temp2[i][1].indexOf("~")>-1)
				{
					...
					inputData2 = "<?xml version='1.0'?><APSelectWithColumnNames_Input><Option>APSelectWithColumnNames</Option><Query>" + QR + "</Query><EngineName>" + wDSession.getM_objCabinetInfo().getM_strCabinetName() + "</EngineName><SessionId>" + wDSession.getM_objUserInfo().getM_strSessionId() + "</SessionId></APSelectWithColumnNames_Input>";*/
	...
	htmlcode=getDynamicHtml_second(outputData,map,WS,CategoryID,SubCategoryID,outputData2,DispColCount,temp2,sMappOutPutXML,wDSession,String sCabName ,String sSessionId , string sJtsIp ,int iJtsPort);
