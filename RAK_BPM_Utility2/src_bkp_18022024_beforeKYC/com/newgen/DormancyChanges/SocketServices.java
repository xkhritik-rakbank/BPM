								...
								else
								{
									...
									if(xmlFolderGetStatus.equalsIgnoreCase("0"))
									{
										...
										for(int field_count=0;field_count<count;field_count++)
										{
											...
											{
												String indexVal = xmlParserGetRecord.getValueOf("IndexValue").trim();
												if(indexVal.contains("'"))
													indexVal = indexVal.replace("'", "''");
												
												String get_columnValues="getdate(),'"+folderIndex+"','"+s_cabinet+"','"+xmlParserGetRecord.getValueOf("IndexId")+"','"+indexVal+"',"
														+ "'N'";
