...
public class DormancyChanges implements Runnable {
	...
	private static List<String> readTextFile(File file) throws IOException  //Read the text file
	{
		...
		{
			char first_letter = line.charAt(0);
			if (first_letter == 'B') 
			{
		...
		try 
		{
			...
			for (File file : listOfFiles)
			{
				...
				if (file.isFile()) 
				{
					...
					if (strDocumentName.contains("ActiveToDormant") || strDocumentName.contains("Active to Dormant"))
						strDocumentName = "ActiveToDormant";
					
					if (strDocumentName.contains("DormantToActive") || strDocumentName.contains("Dormant to Active"))
					...
					String resp = InsertIntoDB.insertToDB(CifDetails, strfullFileName, dateTime, cabinetName,
