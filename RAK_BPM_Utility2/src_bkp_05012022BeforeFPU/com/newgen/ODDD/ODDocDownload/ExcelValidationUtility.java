			...
			if (searchDocMaincode.equalsIgnoreCase("0"))
			{
				...
				if(Record > 0) 
				{
					...
					for (; objWorkList.hasMoreElements(true); objWorkList.skip(true)) 
					{
						...
						if(folderCount.get(vendorName)<=100)
						{
							folderCount.replace(vendorName,folderCount.get(vendorName)+1);
						}
						else
						{
							folderPrefix.replace(vendorName,folderPrefix.get(vendorName)+100);
							folderSuffix.replace(vendorName,folderSuffix.get(vendorName)+100);
...
public void updateMap(String VendorName)
{
	if(!folderCount.containsKey(VendorName))
	{
		...
		folderSuffix.put(VendorName, 100);
	}
}
}