...
<%
	...
	String filePath = properties.getProperty("TWC_LoadImage");
	String debt_acc_num = request.getParameter("debt_acc_num");
	String debtAccArr[] = debt_acc_num.split("#");
	//WriteLog("Delete Sign filePath- " + filePath);
	//WriteLog("Delete Sign debt_acc_num- " + debt_acc_num);
	...
	for (int i = 0;i < debtAccArr.length;i++)
	{
		...
		try {
			...
			for (File child : directoryListing) {
				if (child.getName().contains(fileTempName))
				{
					try {
							child.delete();
							WriteLog("Sucessfully deleted sign file- " + fileTempName);
						}
						catch (Exception e) {
							WriteLog("Error in deleting sign file- " + fileTempName);
						}
				}
			}
		...
		{			
			WriteLog("Error in deleting sign files");
		}
	}
%>