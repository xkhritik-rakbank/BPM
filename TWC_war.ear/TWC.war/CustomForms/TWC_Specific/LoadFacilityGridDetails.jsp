...
<%
	...
	try{
		...
		logger.info("\n wiName "+wiName);
		String table_name="USR_0_TWC_FACILITY_DTLS_GRID";
		logger.info("table_name "+table_name);
		
		String col_Names="Facility_Existing,Facility_Sought,No,Usage,Nature_of_Facility,Purpose,Tenor_Value,Tenor_Frequency,Cash_Margin,InterestType,Interest,InterestMargin,Is_Interest_Below_Grid,Commission,Product_level_Conditions,Remarks";
		...
		Query2 = "SELECT "+col_Names+" FROM "+ table_name+" with (nolock) WHERE WINAME=:WI_NAME ORDER BY cast(LEFT(SUBSTRING(NO, PATINDEX('%[0-9.-]%', NO), 8000),   PATINDEX('%[^0-9.-]%', SUBSTRING(NO, PATINDEX('%[0-9.-]%', NO), 8000) + 'X') -1) as int),NO";
