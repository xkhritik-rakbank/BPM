...
function enabledisable(cntrl)
{
	if (document.forms[0].oth_ecr_dt.options[document.forms[0].oth_ecr_dt.selectedIndex].value.toUpperCase()=="BANK")
	{
		...
		//document.forms[0].oth_ecr_bn.focus();
	}
	else
	{
...
<table id="ECR" border="1" cellspacing="1" cellpadding="1" width=100% >
	...
	<TR>
        ...
        <td nowrap  width="150">
			<input type="text" name="oth_ecr_RB" value='<%=DataFormHT!=null&&DataFormHT.get("oth_ecr_RB")!=null?DataFormHT.get("oth_ecr_RB"):""%>'   size="8" maxlength=5 style='width:150px;' onblur=validateKeys_OnBlur(this,'CSR_OCC_ECR') >
		</td>
        ...
        <td>
					...
					<td  nowrap='nowrap' height ='30' width = 180 class='EWNormalGreenGeneral1' valign="middle"> 
						<input type="button" id="addButtonPendingOptionsECR" class='EWButtonRB NGReadOnlyView' style='width:100px;display:block;margin:auto' onclick="AddPendingOptionsECR('PendingOptionsECR','PendingOptionsSelectedECR');" 
							value="Add >>"><br />
						<input type="button" id="removeButtonPendingOptionsECR" class='EWButtonRB NGReadOnlyView' style='width:100px;display:block;margin:auto'onClick="RemovePendingOptionsECR('PendingOptionsECR','PendingOptionsSelectedECR');"
							value="<< Remove">
					</td>
