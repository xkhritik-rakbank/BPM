...
<%
WriteLog("inside WIIntroduce.jsp = ");
String sInputXML ="";
String sOutputXML = "";
String sCabName="";
...
WFXmlList objWorkList;
	...
	WriteLog("Integration jsp: subProcessShortName    a: "+subProcessShortName);
	String WI_Data="";
	WriteLog("before esapi wi data ");
	WriteLog("wi data orign "+request.getParameter("WIData"));
...
WriteLog("regex replacing "+WI_Data);
	...
	//added by shamshad on 02-02-2022
	...
	WriteLog("WI_Data_arr userEmail :"+userEmail);
	...
	//----------------------------------------------------------------------------------------------------		
		...
		if(!bError)
		{
			...
			if(wicreate.trim().equals("1"))
			{
				...
				if(ProcessName.toString().equalsIgnoreCase("CSR_OCC")&&subProcessShortName!=null&&!subProcessShortName.toString().equals(""))
				{
					//####################################################################################
				...
				else
				{
					...
					else
					{
						...
						//String WI_No = splitString(WI_No_Created);
						String WI_No = "";
						int index = -1;
					...
					//Mail TRIGGER by Aditya.rai	Start 
						WriteLog("Inside mailTrigger methord-----------> ");
						...
						if(ProcessCode.equals("CSR_BT") || ProcessCode.equals("CSR_CCC")|| ProcessCode.equals("CSR_RR") || ProcessCode.equals("CSR_OCC") || ProcessCode.equals("CSR_MR") )
						{
							...
							String tag = "";
							if(ProcessCode.equalsIgnoreCase("CSR_RR") || ProcessCode.equalsIgnoreCase("CSR_CCC")||ProcessCode.equalsIgnoreCase("CSR_OCC") ||ProcessCode.equalsIgnoreCase("CSR_MR"))
							{
							if(ProcessCode.equalsIgnoreCase("CSR_MR"))
							{
							String Amnt = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("CURAmount", request.getParameter("CURAmount"), 1000, true) );
							CUR_Amount = ESAPI.encoder().encodeForSQL(new OracleCodec(), Amnt!=null?Amnt:"");
							WriteLog("WIIntroduction jsp: CUR_Amount    =: "+CUR_Amount);
							String School = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("SchoolName", request.getParameter("SchoolName"), 1000, true) );
							SchoolName = ESAPI.encoder().encodeForSQL(new OracleCodec(), School!=null?School:"");
							WriteLog("WIIntroduction jsp: SchoolName    =: "+SchoolName);
							String Merchnt = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("MerchantName", request.getParameter("MerchantName"), 1000, true) );
							Merchant_Name = ESAPI.encoder().encodeForSQL(new OracleCodec(), Merchnt!=null?Merchnt:"");
							WriteLog("WIIntroduction jsp: Merchant_Name    =: "+Merchant_Name);
							}
							...
							String sProcessName = "";
							if(ProcessCode.equals("CSR_MR") || ProcessCode.equalsIgnoreCase("CSR_OCC"))
							{
							...
							if(ProcessCode.equalsIgnoreCase("CSR_MR") || ProcessCode.equalsIgnoreCase("CSR_OCC"))
							{
							 sProcessName = sProcessName.replaceAll("&#x2f;","/");
							...
							if(ProcessCode.equalsIgnoreCase("CSR_MR") && stage.equalsIgnoreCase("Pending") && (sProcessName.equalsIgnoreCase("LIMIT DECREASE") || sProcessName.equalsIgnoreCase("STATEMENT DATE CHANGE") || sProcessName.equalsIgnoreCase("BLOCKING/UNBLOCKING OF CARD") || sProcessName.equalsIgnoreCase("SECURITY CHEQUE") || sProcessName.equalsIgnoreCase("CLOSURE OF IM") || sProcessName.equalsIgnoreCase("OUTSTANDING ON CREDIT CARD") || sProcessName.equalsIgnoreCase("CLOSURE OF BT/CCC/SC EPP") || sProcessName.equalsIgnoreCase("CLOSURE OF RETAILS EPP") || sProcessName.equalsIgnoreCase("CASHBACK REDEMPTION") || sProcessName.equalsIgnoreCase("Credit Card Closure/Cancellation Request") || sProcessName.equalsIgnoreCase("AIR ARABIA EMAIL IDENTIFIER") || sProcessName.equalsIgnoreCase("DUPLICATE STATEMENT REQUEST")))
							{
								WriteLog("No mail for this case");
							}
							else if(ProcessCode.equalsIgnoreCase("CSR_OCC") && stage.equalsIgnoreCase("Pending") && (sProcessName.equalsIgnoreCase("RE-ISSUE OF PIN") || sProcessName.equalsIgnoreCase("EARLY CARD RENEWAL") || sProcessName.equalsIgnoreCase("CHANGE IN STANDING INSTRUCTIONS") || sProcessName.equalsIgnoreCase("SETUP SUPPL. CARD LIMIT")))
							{
								WriteLog("No mail for this case");
							}
							else if(ProcessCode.equalsIgnoreCase("CSR_BT") && stage.equalsIgnoreCase("Pending"))
								{
								WriteLog("No mail for this case");
								}
							else
							...
							if(ProcessCode.equals("CSR_OCC") || ProcessCode.equals("CSR_MR")){
								String input9 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("subprocess", request.getParameter("subprocess"), 1000, true) );
								String subprocess = ESAPI.encoder().encodeForSQL(new OracleCodec(), input9!=null?input9:"");
								WriteLog("WIIntroduction jsp: Card_No    =: "+subprocess);
								String[] words = subprocess.split("\\s+");
								StringBuilder camelCaseSentence = new StringBuilder();
								for (int i = 0; i < words.length; i++) {
									String word = words[i].toLowerCase();
									camelCaseSentence.append(word.substring(0, 1).toUpperCase()).append(word.substring(1));
									if (i < words.length - 1) {
										camelCaseSentence.append(" ");
									}
								}
									String subReq = camelCaseSentence.toString();
								emailBody = emailBody.replaceAll("#Sub_Process_Name#", subReq);
							}
							if(ProcessCode.equals("CSR_RR")){
								String reversal = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("reversalFor", request.getParameter("reversalFor"), 1000, true) );
								String reversal_For = ESAPI.encoder().encodeForSQL(new OracleCodec(), reversal!=null?reversal:"");
								WriteLog("WIIntroduction jsp: reversal_For    =: "+reversal_For);
								if(!reversal_For.equalsIgnoreCase("Others Pls. Specify"))
								{
								emailBody = emailBody.replaceAll("#Reversal_For#", reversal_For);
								}
								else
								{
						...
						//SMS trigger
							...
							if(ProcessCode.equals("CSR_OCC") || ProcessCode.equals("CSR_MR")){
								...
								String subprocess = ESAPI.encoder().encodeForSQL(new OracleCodec(), input9!=null?input9:"");
								WriteLog("WIIntroduction jsp: Card_No    =: "+subprocess);
								String[] words = sProcessName.split("\\s+");
								StringBuilder camelCaseSentence = new StringBuilder();
								for (int i = 0; i < words.length; i++) {
									String word = words[i].toLowerCase();
									camelCaseSentence.append(word.substring(0, 1).toUpperCase()).append(word.substring(1));
									...
									String subReq = camelCaseSentence.toString();
								txtMessage = txtMessage.replaceAll("#Sub_Process_Name#", subReq);
							}
							if(ProcessCode.equals("CSR_RR"))
							{
								String reversal = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("reversalFor", request.getParameter("reversalFor"), 1000, true) );
								...
								if(!reversal_For.equalsIgnoreCase("Others Pls. Specify"))
								{
								txtMessage = txtMessage.replaceAll("#Reversal_For#", reversal_For);
								}
								else
								{
