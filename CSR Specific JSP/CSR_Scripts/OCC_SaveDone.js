function checkMonthDiff1()
{
	...
	if(objDataForm.CCI_ExpD.value==""||objDataForm.oth_ecr_RB.value=="")
		return true;
	
	var month1=objDataForm.CCI_ExpD.value.substring(0,2);
	var year1=objDataForm.CCI_ExpD.value.substring(3,5);
...
function OCC_SaveDone(chkMandatory)
{
	 var objWI_Obj=window.top.wi_object;
	 var objDataForm=window.parent.frames['left'].document.forms["dataform"];
/*
	...
	Module          :       Other Credit Card Request
	File            :       OCC_SaveDone.js
...
*/
		...
		if(chkMandatory==1&&objDataForm.CCI_ExtNo.value==""){
			...
			objDataForm.CCI_ExtNo.focus();
			return false;
		}else if(chkMandatory==1&&objDataForm.CCI_ExtNo.value.length < 4){
		...
		}else if(chkMandatory==1&&objDataForm.CCI_CCRNNo.value.length < 9){
		...
		if(processName=="CSR_BT"||processName=="CSR_CCC")
		{
			if(chkMandatory==1&&objDataForm.CCI_SC.value==""){
				...
				objDataForm.CCI_SC.focus();
				return false;
			}else if(chkMandatory==1&&objDataForm.CCI_SC.value.length < 7){
...
*/				
			...
			{	
				...
				if(objDataForm.oth_ecr_RB.value!="" && !checkMonthDiff1())
				{					
