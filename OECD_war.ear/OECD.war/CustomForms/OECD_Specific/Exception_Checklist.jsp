...
// Date written (DD/MM/YYYY) : 29-Nov-2017
...
<%!
	...
	{
		WriteLog("Start of load exceptions");
		...
		try{
			...
			String subXML ="";
			String Query = "select ExcpCode,WSName,UserName,Decision,ActionDateTime from USR_0_OECD_EXCEPTION_HISTORY where winame='"+WINAME+"' order by ExcpCode asc, cast(convert(varchar(24),actiondatetime+':000',113) as datetime) desc";
			...
			Query="Select Checklist_Item_Desc,Checklist_Item_Code,Routing_Unit_1_Branches,Routing_Unit_1_WM_SME,Approving_Unit from USR_0_Rak_Checklist_Master where IsActive='Y' AND wsname='"+WSNAME+"' AND canView= 'Y' and processname='OECD' order by id Asc";
			...
			if(mainCodeValue.equals("0"))
			{
				...
				for(objWorkList.reInitialize(true);objWorkList.hasMoreElements(true);objWorkList.skip(true))
				{
					...
					else if("OPS_CHECKER,".indexOf(WSNAME.toUpperCase()+",")>-1)
					//Raise New
					{
					...
					WriteLog("htmlraised inside loop "+htmlraised);	
					WriteLog("htmlunraised inside loop "+htmlunraised);							
				}	
				WriteLog("htmlraised "+htmlraised);
				WriteLog("htmlunraised "+htmlunraised);
				WriteLog("htmlraised + htmlunraised "+(htmlraised + htmlunraised));
...
//**************************************************************************************************//		
	public String getRow(String itemCode,String raisedExceptions,String relatedWorkstep,String approvalUnit,String WSNAME,String item,HashMap<String,String> canRaiseMap,HashMap<String,String> canClearMap)
	{
		...
		//Showing Exception History table values in below worksteps
		if(WSNAME.equalsIgnoreCase("OPS_Maker") || WSNAME.equalsIgnoreCase("Control") || WSNAME.equalsIgnoreCase("System_Update"))
		{
...
<%
	...
	String query="Select Checklist_Item_Desc,Checklist_Item_Code,Routing_Unit_1_Branches,Routing_Unit_1_WM_SME,Approving_Unit,canRaise,canClear from USR_0_Rak_Checklist_Master where IsActive='Y' and wsname = '"+wsname+"' and canView= 'Y' and processname='OECD' order by id Asc";
