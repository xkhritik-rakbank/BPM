...
public class EventHandler extends CMSCommon  implements IFormServerEventHandler
{	
	...
	{	
		//CMS.setLogger();
		CMS.mLogger.info("Inside executeServerEvent() ak 101 ---control: " + control + "\nevent: " + event + "\nStringData: "
				+ Stringdata);
		...
		switch(event)
		{
			case "FORMLOAD": return new CMS_FormLoad().formLoadEvent(iformObj,control,event,Stringdata);
			case "CLICK" : return new CMS_Click().clickEvent(iformObj,control,Stringdata);
			case "INTRODUCEDONE" :	 return new CMS_IntroDone().onIntroduceDone(iformObj,control,event,Stringdata);
			default: return "";
		}
	}
