...
(function (factory) {
  ...
  $.FE.PLUGINS.wordPaste = function (editor) {
    ...
    function _buildList (node, head_style_hash) {
      ...
      if (next_element_sibling != null) {
        previous_element_sibling = next_element_sibling.previousElementSibling;
      }
      ...
      while (next_element_sibling) {
        ...
        if (!_isList(next_element_sibling)) {
          ...
          // Skip bookmarks.
          if (next_element_sibling.outerHTML && next_element_sibling.outerHTML.indexOf('mso-bookmark') > 0 && (next_element_sibling.textContent || '').length == 0) {
        ...
        if (next_level > level) {
          ...
          if (next_element_sibling.firstElementChild && next_element_sibling.firstElementChild.firstElementChild && next_element_sibling.firstElementChild.firstElementChild.firstChild) {
            order_regex.lastIndex = 0
            next_order = order_regex.test(next_element_sibling.firstElementChild.firstElementChild.firstChild.data || '');
          }
          ...
          if (previous_element_sibling && previous_element_sibling.firstElementChild && previous_element_sibling.firstElementChild.firstElementChild && previous_element_sibling.firstElementChild.firstElementChild.firstChild) {
            order_regex.lastIndex = 0
            prev_order = order_regex.test(previous_element_sibling.firstElementChild.firstElementChild.firstChild.data || '');
          }
