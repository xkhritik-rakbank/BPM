  ...
  $.FE.RegisterQuickInsertButton('image', {
    ...
    callback: function () {
      ...
      if (!editor.shared.$qi_image_input) {
        editor.shared.$qi_image_input = $('<input accept="image/' + editor.opts.imageAllowedTypes.join(', image/').toLowerCase() + '" name="quickInsertImage' + this.id + '" style="display: none;" type="file">');
  ...
  $.FE.PLUGINS.quickInsert = function (editor) {
    ...
    function _place($tag) {
      ...
      if (editor.opts.iframe) {
        qiTop += editor.$iframe.offset().top;
      }
