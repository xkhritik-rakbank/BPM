...
if (reqType.equals("SendMail"))
{
	...
	WriteLog("\nSendMail wsname: "+wsname);
	WriteLog("\nSendMail decision: "+decision);
	WriteLog("\nSendMail processname"+processname);
	
	String sInputXML="<?xml version=\"1.0\"?>" +                                                           
		"<APProcedure2_Input>" +
		"<Option>APProcedure2</Option>" +
		"<ProcName>"+"NG_TF_CUST_MAIL_PROC"+"</ProcName>"+
...
{
	String table_name = "USR_0_TF_SUBCATEGORY";
	String query = "" ;
	String outputXML ="";
	String InputXml = "";
...
//added by stutee.mishra to get isDistributed value from DB 
else if (reqType.equals("fetchIsDistributed"))
{
	String table_name = "RB_TF_EXTTABLE";
	String query = "" ;
	String outputXML ="";
	String InputXml = "";
	String sJtsIp = customSession.getJtsIp();
	...
	String sessionID = customSession.getDMSSessionId();
	String params = "";
	query = "select isDistributed from "+table_name+" with(nolock) WHERE WI_NAME=:WorkitemName" ;
	params = "WorkitemName=="+WorkitemName;
	outputXML ="";
	...
	WriteLog( "InputXml Fetch isDistributed----"+InputXml);
	outputXML=WFCustomCallBroker.execute(InputXml, sJtsIp, iJtsPort, 1);
	WriteLog( "\noutputXML Fetch isDistributed----"+outputXML);

	String isDistributedValue = "";
	if(outputXML.indexOf("<isDistributed>")>-1)
	{
		isDistributedValue=outputXML.substring(outputXML.indexOf("<isDistributed>")+"<isDistributed>".length(),outputXML.indexOf("</isDistributed>"));
	}
    WriteLog( "\nValue isDistributedValue----"+isDistributedValue);
	out.clear();
	out.println(isDistributedValue);
}	
// added by stutee.mishra ends.
else if (reqType.equals("opsmaker_remarks"))
{
	String table_name = "USR_0_TF_WIHISTORY";
	String query = "" ;
	String outputXML ="";
	String InputXml = "";
	String sJtsIp = customSession.getJtsIp();
...
else if (reqType.equals("GetOpsRejectRemarks"))
{
	...
	String query1="select item_code,Item_Desc from USR_0_TF_Error_Desc_Master with(nolock) where isactive=:isactive order by id asc";
	...
	String strQuery = "Select top 1 remarks,rejectreasons from usr_0_TF_wihistory where actual_wsname =:actual_wsname and winame=:winame order by entrydatetime desc";
...
else if (reqType.equals("fetchdecision") )
{
							...
							String Query="select DECISION from USR_0_TF_DECISION_MASTER WHERE WORKSTEP_NAME=:WORKSTEP_NAME and ROUTECATEGORY =:ROUTE_CATEGORY";
