...
public class EventHandler extends CMPCommon  implements IFormServerEventHandler
{	
	...
	{	
		CMP.mLogger.info("Inside executeServerEvent() ak 201-1 ---control: " + control + "\nevent: " + event + "\nStringData: "
				+ Stringdata);
		...
		switch(event)
		{
			case "FORMLOAD": return new CMP_FormLoad().formLoadEvent(iformObj,control,Stringdata);				
			case "CLICK" : return new CMP_Click().clickEvent(iformObj,control,event,Stringdata);
			case "INTRODUCEDONE" :	 return new CMP_IntroDone().onIntroduceDone(iformObj, control, Stringdata);
			default: return "";
		}
	}
