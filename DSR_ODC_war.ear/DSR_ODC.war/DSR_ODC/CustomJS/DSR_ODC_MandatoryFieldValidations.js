function validateMandatoryForPending() 
{
	...
	if ((getValue("CardNo_Text") != null) && (getValue("CardNo_Text") != ""))
	{
		setFocus("CardNo_Text");
		return false;
	}
	/*if (!CheckValueForField("DCI_ExtNo", getValue("DCI_ExtNo"), "Ext. No.", "digits"))
		return false;*/
	//alert("DCI_ExtNo :"+getValue("DCI_ExtNo"));
	if ((getValue("DCI_ExtNo") != "") && (getValue("DCI_ExtNo").length != 4))
	{
...
/*	if (getValue("oth_ecr_RB") != "")
	{
		...
		try 
		{
			if ((getValue("DCI_ExpD") != "") && (getValue("DCI_ExpD") != "Null") && (getValue("DCI_ExpD") != null))
			{
...
function validateMandatory()
{
	if ((ActivityName.toUpperCase().indexOf("INTRODUCTION") > -1) || (ActivityName == "Pending") || (ActivityName == "Branch_Return"))
	{
		if (ActivityName.toUpperCase().indexOf("INTRODUCTION") > -1)
		{
			...
			{
				showMessage("CardNo_Text", "Please enter card no and click refresh button to fetch card data!","error");
				setFocus("CardNo_Text");
				return false;
			}
		}
		...
		{
			if ((getValue("CardNo_Text") != null) && (getValue("CardNo_Text") != "")&& (!validateCCNo(getValue("CardNo_Text"), "CardNo_Text")))
			{
				setFocus("CardNo_Text");
				return false;
			}
			...
			/*if (!CheckValueForField("DCI_ExtNo", getValue("DCI_ExtNo"), "Ext. No.", "digits"))
				return false;*/
			if (getValue("DCI_ExtNo").length != 4)
			{
				showMessage("DCI_ExtNo", "Ext No. should be of 4 digits!","error");
				setFocus("DCI_ExtNo");
				return false;
			...
			/*	if (getValue("oth_ecr_RB").length < 5)
				...
				var m = 0;
				try {
					alert("DCI_ExpD : " + getValue("DCI_ExpD"));
					if ((getValue("DCI_ExpD") != "") && (getValue("DCI_ExpD") != "Null") && (getValue("DCI_ExpD") != null))
					{
						i = parseInt(getValue("DCI_ExpD").substring(0, 2));
						j = parseInt(getValue("DCI_ExpD").substring(3, 5));
