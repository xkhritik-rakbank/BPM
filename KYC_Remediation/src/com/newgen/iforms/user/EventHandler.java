...
public class EventHandler extends KYC_Remediation_Common  implements IFormServerEventHandler
{	
	...
	{	
		...
		KYC_Remediation.mLogger.info("Inside executeServerEvent() ak 101 ---control: " + control + "\nevent: " + event + "\nStringData: "
				+ Stringdata);
		...
		{
			try {
				return new KYC_Remediation_Click().clickEvent(iformObj,control,Stringdata);
		...
		{
			return new KYC_Remediation__FormLoad().formLoadEvent(iformObj,control,event,Stringdata);
		}
		else if("introducedone".equalsIgnoreCase(event))
		{
			return new KYC_Remediation_IntroDone().onIntroduceDone(iformObj,control,event,Stringdata);
		}
		else if("Change".equalsIgnoreCase(event))
		{
			return new KYC_Remediation_Change().changeEvent(iformObj,control,event,Stringdata);
		}
		else
		{
			return "unhandled";
