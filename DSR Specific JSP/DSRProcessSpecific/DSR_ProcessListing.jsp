...
function clearFrames(){
	try{
		window.parent.frames['frmData'].document.location.href="DSR_blank_withLogo.jsp";
...
<%
	...
	try{
		...
		while(sOutputXMLQueueList_loop.indexOf("<MainCode>18</MainCode>") == -1)
		{
			...
			WriteLog("WMGetQueueList Input ProcessListing.jsp: "+sInputXML);
		...
		String[] SubQueueArr=null;       // It will keep all the DSR_OCC work introduction queuenames
		...
		try{
			...
			properties.load(new FileInputStream(strFilePath + fileSeparator+"DSR_RB.properties"));
		...
		try{
			...
			for (RecordList =  xmlResponse.createList("QueueList", "QueueInfo");RecordList.hasMoreElements();){
				...
				//out.println(RecordList.getVal("Name").toUpperCase());
					...
					else if(RecordList.getVal("Name").toUpperCase().indexOf("DSR_ODC_")!=-1)
					{
		...
		while(st.hasMoreTokens()) {
			...
			while(strProcessNameCodeList.indexOf(";")!=-1){
				...
				if(status.equals("0")){
					...
					WriteLog(" sProcessName inn DSR ProcessListing:"+strProcessName);%>	
					<%
					...
					{
						out.println("d.add("+count+","+localcount+",'"+sServiceName+"','DSR_Initiate_Top.jsp?BranchName="+URLEncoder.encode(strBranchName,"UTF-8") + "&BranchCode="+URLEncoder.encode(sBranch_Code.substring(1,sBranch_Code.length()),"UTF-8") + "&ProcessCode="+strProcessCode + "&ProcessName="+strProcessName + "&strCSMMailId="+strCSMMailId+"&strBMailId="+strBMailId+"','','frmData');");
						WriteLog("---After DSR_Initiate_Top line---");
					}
				}
			}	
