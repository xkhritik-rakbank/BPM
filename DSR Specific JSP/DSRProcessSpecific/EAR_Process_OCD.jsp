...
/*			else if (document.getElementById("limitAED").value=='')
			...
			else if (document.getElementById("excessAuthorizationDept").value=='')
			{
				alert('Please select Excess Authorization Department');
				document.getElementById("excessAuthorizationDept").focus();
				return false;
			}
			else if (document.getElementById("excessAuthorizedBy").value=='')
			{
				alert('Excess Authorized By cannot be blank');
				document.getElementById("excessAuthorizedBy").focus();
				return false;
			}
			else if (document.getElementById("excessAuthorizationDate").value=='')
			{
				alert('Please select Excess Authorization Date');
				document.getElementById("excessAuthorizationDate").focus();
				return false;
			}
...
<%
	...
	ArrayList approvalDepartment = new ArrayList();
	try
	{
		String lstrQuery = "select DepartmentName from RB_EAR_APPROVAL_DEPT where 1=:ONE";
		String params ="ONE==1";
		...
		else if(Integer.parseInt(sOutputXML.substring(sOutputXML.indexOf("<MainCode>")+10, sOutputXML.indexOf("</MainCode>")))==0) 
		{
			...
			{
				String departmentName=RecordList1.getVal("DepartmentName");
				approvalDepartment.add(departmentName);
			}
	
			/*
			String lstrRecordsXML =	sOutputXML.substring(sOutputXML.indexOf("<Records>") , sOutputXML.indexOf("</Records>")+10);
			if (Integer.parseInt(sOutputXML.substring(sOutputXML.indexOf("<TotalRetrieved>")+16, sOutputXML.indexOf("</TotalRetrieved>")))==0)
			{
			}
			else
			{
				int retrievedCount = Integer.parseInt(sOutputXML.substring(sOutputXML.indexOf("<TotalRetrieved>")+16, sOutputXML.indexOf("</TotalRetrieved>")));
				for (int i=0; i<retrievedCount; i++)
				{
					String temp = lstrRecordsXML.substring(lstrRecordsXML.indexOf("<td>")+4, lstrRecordsXML.indexOf("</td>"));
					approvalDepartment.add(temp);
					if (retrievedCount!=i+1)
					{
						lstrRecordsXML = lstrRecordsXML.substring((lstrRecordsXML.indexOf("</td>")+5),lstrRecordsXML.length());
					}
				}
			}
			*/
		...
		{
			WriteLog("<Exception>Some Problem</Exception>");
		}
	}
	catch(Exception e)
	{
		WriteLog("Exception in fetching Department Name");
	}
...
<BODY topmargin=0 leftmargin=15 class='EWGeneralRB'>
	<table width=100% border=0>
		<tr width=100%>
			<td width=60% class="EWLargeLabel" align=right valign=center><center>Excess Authorization Request (For Cases Approved Outside Credit Department) Initiation Module</center></td>
	...
	<form name="dataform"> 
		...
		<table border="1" cellspacing="1" cellpadding="1" width=100% >
			...
			<TR>
				...
				<td nowrap width="150" height="16" class="EWLabelRB" id="excessAuthorizationDeptLabel">Excess Authorization Department</td>
				<td nowrap width="180">
					<select name="excessAuthorizationDept" id="excessAuthorizationDept" style='width:150px;'>
						<option value="">--Select--</option>
<%
						Iterator itr = approvalDepartment.iterator();
						while (itr.hasNext())
						{
							String strVal = itr.next().toString();
%>
...
%>
			...
			<tr>
				<td nowrap width="150" height="16" class="EWLabelRB" id="excessAuthorizedByLabel">Excess Authorized By</td>
				<td nowrap  width="150"><input type="text" name="excessAuthorizedBy" size="8" maxlength=70 onKeyUp="validateAlphaNumericDataOnKeyUp(this);" style='width:150px;' ></td>
				<td nowrap width="150" height="16" class="EWLabelRB" id="excessAuthorizationDateLabel">Excess Authorization Date</td>
				<td nowrap  width="150"><input type="text" id="excessAuthorizationDate" name="excessAuthorizationDate" size="8" readonly maxlength=70 style='width:125px;'>&nbsp;&nbsp;<a href='1' onclick = "initialise('excessAuthorizationDate');return false;" target='_parent' ><img src='/webdesktop/webtop/images/images/cal.gif' width='16' height='16' border='0' alt='Click here to pick up the date' ></a></td>
			</tr>
			<TR>
			...
			<TR>
				...
				<td nowrap width="150" height="16" class="EWLabelRB" id="RemarksLabel">Remarks</td>
				<td nowrap width="150"><textarea id="Remarks" name="Remarks" value="" cols=18  rows=2  size="250" onKeyup="CheckMxLength(this,250);" maxlength="250" ></textarea></td>
			</tr>		
		</table>
		<input type="hidden" name="ProcessName" id="ProcessName" value="EAROutOfCD">
	</form>
	<form>
	<table border=1 cellspacing=1 cellpadding=1 width=100%>
		<tr width=100%>
