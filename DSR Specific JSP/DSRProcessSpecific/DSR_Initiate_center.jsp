...
<% 
	...
	String input1 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("DebitCardNo", request.getParameter("DebitCardNo"), 1000, true) );
	String DebitCardNo = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1!=null?input1:"");
	WriteLog("Integration jsp: DebitCardNo: "+DebitCardNo);
	WriteLog("Integration jsp: DebitCardNo 1: "+request.getParameter("DebitCardNo"));

	String input2 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("txtDebitCardNo", request.getParameter("txtDebitCardNo"), 1000, true) );
	String txtDebitCardNo = ESAPI.encoder().encodeForSQL(new OracleCodec(), input2!=null?input2:"");
	WriteLog("Integration jsp: txtDebitCardNo: "+txtDebitCardNo);
	WriteLog("Integration jsp: txtDebitCardNo 1: "+request.getParameter("txtDebitCardNo"));
...
function validateProcessData()
{
		...
		if(processName=="DSR_ODC") 
		{
			...
			if(dataForm.request_type.value=="Credit Limit Increase" || dataForm.request_type.value=="Change in Standing Instructions" || dataForm.request_type.value=="Card Upgrade" ||dataForm.request_type.value=="Setup Suppl. Card Limit" || dataForm.request_type.value=="Credit Shield" ){
				if (!(dataForm.DCI_DebitCN.value.charAt(13)==0 ) )	{
					alert("Only Primary Cards Allowed");
					return false;
				}
			}
			if((dataForm.DCI_CT.value == 'L') && (dataForm.request_type.value=="Re-Issue of PIN" ||dataForm.request_type.value=="Card Replacement" ||dataForm.request_type.value=="Early Card Renewal" ||dataForm.request_type.value=="Transaction Dispute"  ||dataForm.request_type.value=="Card Delivery Request" )){
				alert("Current Card Type is not allowed for this request");
				return false;
			}
	...
	if(processName=="DSR_CBR")
	{	
	    /*COMMENTED by Amandeep 17 jan 2010
		if (dataform.DCI_CCRNNo.value.charAt(7)=='0'&&dataform.DCI_CCRNNo.value.charAt(8)=='0')
		{
		}
		else
		{
...

     {
		...
		if(trim(dataForm.DCI_ExtNo.value)==""){
			alert("Ext No. is Mandatory");
			dataForm.DCI_ExtNo.value="";
			dataForm.DCI_ExtNo.focus();
			return false;
		}else if(!CheckDataType(trim(dataForm.DCI_ExtNo.value),"Numeric")){
			
			alert("Invalid Data in Ext No. Only Numeric are allowed");
			dataForm.DCI_ExtNo.value="";
			dataForm.DCI_ExtNo.focus();
			return false;
		}else if(trim(dataForm.DCI_ExtNo.value).length < 4){
...
*/
 ...
 if(processName=="DSR_CBR")
        {	
		    //alert("In DSR.CBR--"+dataForm.AMOUNT.value);
			...
			/*if(!CheckDataType(trim(dataForm.AMOUNT.value),"Numeric")){
			
			//alert("Invalid Amount No. Only Numeric are allowed");
...
if(processName=="DSR_DCB")
	...
	if(processName=="DSR_ODC")
	{		
	     //alert("The Name..... :"+srequest_type+"::"+processName);
		...
		//alert("The Name..... :"+srequest_type+"::"+processName);
			...
			{	
				...
				if(dataForm.oth_ecr_RB.value!="")
				{
					var month1=dataForm.DCI_ExpD.value.substring(0,2);
					var year1=dataForm.DCI_ExpD.value.substring(3,5);
			...
			{	
				...
				{					
					...
					dataForm.oth_td_Amount.value="";
					dataForm.oth_td_Amount.focus();
					return false;
				}
				if(!CheckDataType(trim(dataForm.oth_td_Amount.value),"Numeric")){
			...
			{	
				if(dataForm.DCI_CT.value.toUpperCase()=="T")
				{					
			...
			{	//Suplllementary Card No. validation to be developed
				...
				if(dataForm.oth_ssc_SCNo.options.value.length=="")
				{					
			...
			{	
				...
				if(dataForm.oth_cs_DSR.checked==true&&dataForm.oth_cs_Amount.value=="")
				{					
					alert("Amount is Mandatory");
				...
				if(dataForm.oth_cs_DSR.checked==true&&!CheckDataType(replaceAll(dataForm.oth_cs_Amount.value,",",""),"numeric82"))
				{					
					alert("Only Numerics(10,2) are allowed in Amount");
					dataForm.oth_cs_Amount.focus();
					return false;
...
function validateProcessDataForPending()
{
		...
		var dataForm=window.document.forms['dataform'];
			//alert(dataForm.DCI_CCRNNo.value.length);
		 ...
		 if(!CheckDataType(trim(dataForm.DCI_ExtNo.value),"Numeric")){
			alert("Invalid Data in Ext No. Only Numeric are allowed");
			dataForm.DCI_ExtNo.focus();
			return false;
		}
		...
		//Amandeep Check
		if(processName=="DSR_DCB" || processName=="DSR_CBR")
			{
			if(dataForm.DCI_CT.value == 'L'){
				alert("Current Card Type is not allowed for this request");
				return false;
		...
		if(processName=="DSR_ODC")
	{
		...
		var srequest_type=dataForm.request_type.options[dataForm.request_type.selectedIndex].value;
			...
			//alert("The Type is:"+srequest_type);
			if(srequest_type=="Re-Issue of PIN")
			{	
			     //alert("Inside...");
				if(dataForm.oth_rip_reason.options[dataForm.oth_rip_reason.selectedIndex].value=="")
...
*/
	...
	if(dataForm.oth_ssc_RR.value.length>500)
		...
		if(processName=="DSR_ODC") {
			...
			//alert("test card type:"+dataForm.DCI_CT.value);
			if((dataForm.DCI_CT.value == 'L') && (dataForm.request_type.value=="Re-Issue of PIN" ||dataForm.request_type.value=="Card Replacement" ||dataForm.request_type.value=="Early Card Renewal" ||dataForm.request_type.value=="Transaction Dispute"  ||dataForm.request_type.value=="Card Delivery Request" )){
				alert("Current Card Type is not allowed for this request");
					return false;
					//return true;
		 ...
		 if(!CheckDataType(dataForm.DCI_ExtNo.value,"Numeric")){
...
{
	var dataForm=window.document.forms['dataform'];
	if(dataForm.DCI_ExpD.value==""||dataForm.oth_ecr_RB.value=="")
		return true;
	var month1=dataForm.DCI_ExpD.value.substring(0,2);
	var year1=dataForm.DCI_ExpD.value.substring(3,5);
...
function introduce(args)
{
	...
	url = "/webdesktop/CustomForms/DSRProcessSpecific/WIIntroduce.jsp";
	//alert('2');
	//for mail and SMS trigger by Aditya.rai --Start
	var ProcessCode = dataForm.ProcessCode.value;
	if(ProcessCode == 'DSR_ODC'){
		var Mobile_No = dataForm.DCI_MONO.value;
		var Amount = dataForm.oth_td_Amount.value;
		var subprocess = dataForm.request_type.value;
		var pendingfor = dataForm.PendingOptionsFinal.value;
		var Card_No = window.document.getElementById("DCI_DebitCN").value.substring(12,16);
		var param="&ProcessName="+processName+"&subProcessShortName="+window.document.getElementById("subProcessShortName").value+"&WIData="+window.document.getElementById("WIData").value+"&Amount="+Amount+"&pendingfor="+pendingfor+"&subprocess="+subprocess+"&Mobile_No="+Mobile_No+"&ProcessCode="+ProcessCode+"&Card_No="+Card_No+"&args="+args;
	}
	else if(ProcessCode == 'DSR_MR'){
		var Mobile_No = dataForm.DCI_MONO.value;
		var Amount = dataForm.DCI_ExtNo.value;
		var Card_No = window.document.getElementById("DCI_DebitCN").value.substring(12,16);
		var param="&ProcessName="+processName+"&subProcessShortName="+window.document.getElementById("subProcessShortName").value+"&WIData="+window.document.getElementById("WIData").value+"&Amount="+Amount+"&Mobile_No="+Mobile_No+"&ProcessCode="+ProcessCode+"&Card_No="+Card_No+"&args="+args;
	}
	else{
		var param="&ProcessName="+processName+"&subProcessShortName="+window.document.getElementById("subProcessShortName").value+"&WIData="+window.document.getElementById("WIData").value+"&ProcessCode="+ProcessCode;
	}
	//End
...
{
	//alert("123--");
	window.parent.frames['frameClose'].document.location.href="DSR_initiate_bottom.jsp";
	window.document.forms["dataform"].DCI_ExtNo.focus();
	var processName='<%=URLDecoderProcessName%>';
	//alert(processName);
	//document.write(processName);
	if(processName=="Debitcard Service Request - Other Debit Card Requests")
	{
		//alert("1239999999--");
...
<form name="dataform">
<%String sProcessName=URLDecoderProcessName;%>

<%String sCCN=DebitCardNo.toString();%>
<% 
   String vCardno = txtDebitCardNo; 
   String temp[] = vCardno.split("-");
   String total="";
...
<jsp:include page="DSR_CommonBlocks.jsp">
 <jsp:param name="cards" value="<%=total%>"/>
<jsp:param name="DebitCN" value="<%=sCCN%>"/>
<jsp:param name="ProcessName" value="<%=sProcessName%>"/>
</jsp:include>

<%if(sProcessName.equals("Debitcard Service Request - Cash Back Request")) 
	   {%>
<jsp:include page="../DSRProcessSpecific/DSR_ProcessSpec_CBR.jsp" >
<jsp:param name="DebitCN" value="<%=sCCN%>"/>
</jsp:include>

<%}else if(sProcessName.equals("Debitcard Service Request - Debit Card Blocking Request")) {%>
<jsp:include page="../DSRProcessSpecific/DSR_ProcessSpec_DCB.jsp" >
<jsp:param name="DebitCN" value="<%=sCCN%>"/>
</jsp:include>
<%}
 
 else if(sProcessName.equals("Debitcard Service Request - Miscellaneous Requests")) {

 %>
<jsp:include page="../DSRProcessSpecific/DSR_ProcessSpec_MR.jsp">
<jsp:param name="DebitCN" value="<%=sCCN%>"/>
</jsp:include>
<%}
...
<input type="text" id="PendingForCDR"name="PendingForCDR" style="visibility:hidden" >
...
<input type="text" id="PendingForTD"name="PendingForTD" style="visibility:hidden" >
<input type=text readOnly name="ProcessCode" id="ProcessCode" value='<%=URLDecoderProcessCode%>' style='display:none'>
<input type=text readOnly name="DCI_DebitCN" id="DCI_DebitCN" value='<%=DebitCardNo%>' style='display:none'>
