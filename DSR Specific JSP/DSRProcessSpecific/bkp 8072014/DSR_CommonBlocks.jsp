...
<%@ include file="../DSRProcessSpecific/Log.process"%><%@ page import="java.text.SimpleDateFormat" %><%@ page import="java.io.*,java.util.*"%><%@ page import="java.text.*"%><%@ page import="java.net.*"%><%@ page import="java.lang.String.*"%><%@ page import="java.lang.Object"%><%@ page import="com.newgen.wfdesktop.xmlapi.*" %><%@ page import="com.newgen.wfdesktop.util.*" %>
...
<jsp:useBean id="wfsession" class="com.newgen.wfdesktop.session.WFSession" scope="session"/><script language="javascript" src="/webdesktop/webtop/en_us/scripts/DSR_RBCommon.js"></script><script>
		...
		if(request.getParameter("ProcessInstanceId")==null)
		{
			...
			String strCompCode="",strReasonCode="";	
				...
				sInputXML =	"?xml version=\"1.0\"?>\n" +
					...
					sInputXML =	sInputXML + "<CardNo>"+request.getParameter("DebitCN")+"</CardNo>";
...
responseMsg.substring(responseMsg.indexOf("<CardSubType>")+"<CardSubType>".length(),responseMsg.indexOf("</CardSubType>"));
				...
				else
				{
					...
					WriteLog("Card status is not NORM");
					out.println("<script>alert(\"Card status is not NORM. Cannot proceed with the request.\")</script>");
					out.println("<script>window.parent.frames['frameProcess'].document.location.href=\"DSR_blank.jsp\"; window.parent.frames['frameClose'].document.location.href=\"DSR_blank.jsp\";</script>");
					}
			...
			WriteLog("Inside...A "+URLDecoder.decode(request.getParameter("ProcessName")));
			if(URLDecoder.decode(request.getParameter("ProcessName")).equalsIgnoreCase("Debitcard Service Request - Cash Back Request"))
			{
		...
		{
			WriteLog("Inside1...");
			String sInputXML1="<?xml version=\"1.0\"?><WMFetchProcessInstanceAttributes_Input><Option>WMFetchProcessInstanceAttributes</Option><EngineName>"+sCabname+"</EngineName><SessionId>"+sSessionId+"</SessionId><ProcessInstanceId>"+request.getParameter("ProcessInstanceId")+"</ProcessInstanceId></WMFetchProcessInstanceAttributes_Input>";
...
<% String temp = request.getParameter("cards");
  ...
  <%if(request.getParameter("ProcessInstanceId")!=null){ 
	...
	<%}%>
		...
		<TR>
			...
			<td nowrap  width="150"><input type="text" name="DCI_CName" value='<%=sCustomerName%>' disabled size="8" maxlength=90 style='width:150px;' ></td>
			<td nowrap width="190" height="16" class="EWLabelRB" id="DCI_ExpD">Expiry Date</td>
			<td><input type="text" name="DCI_ExpD" value='<%=sExpiryDate%>'  disabled size="8" maxlength=5 style='width:150px;'></td>
		</tr>
		<TR>
	...
	<table border="1" cellspacing="1" cellpadding="1" width=100% >
		...
		<tr>
			<td nowrap  height="16" class="EWLabelRB" id="C_VD_TINCheck" colspan=3><input type="checkbox" name="VD_TINCheck" style='width:25px;' >&nbsp;TIN Check</td>
		</tr>
		<table border="1" cellspacing="1" cellpadding="1" width=100% >
			<TR>
	...
	<%if(request.getParameter("ProcessInstanceId")==null && URLDecoder.decode(request.getParameter("ProcessName")).equalsIgnoreCase("Debitcard Service Request - Other Debit Card Requests"))
	{ %>
		<table border="1" cellspacing="1" cellpadding="1" width=100% >		
			<tr class="EWHeader" width=100% class="EWLabelRB2">
	...
	<%if(request.getParameter("ProcessInstanceId")==null){%>
	<input type=text readOnly name="USER_BRANCH" id="USER_BRANCH" value='<%=request.getParameter("BranchCodeName").substring(0,request.getParameter("BranchCodeName").indexOf("+"))%>' style='display:none'>
	<%}%>
