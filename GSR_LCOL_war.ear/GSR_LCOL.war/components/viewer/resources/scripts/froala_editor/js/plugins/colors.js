  ...
  $.FE.PLUGINS.colors = function (editor) {
    ...
    function _showColorsPopup () {
      ...
      if (!$popup.hasClass('fr-active')) {
        ...
        // Refresh both tabs selected color.
    ...
    function _updateColor (val) {
      var $popup = editor.popups.get('colors.picker');
      var $selectionColor = $popup.find('.fr-' + val + '-color .fr-active-item').attr('data-param1');
      var $input = $popup.find('.fr-color-hex-layer input');
      var tab = $popup.find('.fr-colors-tab[data-param1="' + val + '"]');

      if ($input.length && tab.hasClass('fr-selected-tab')) {
        $input.val($selectionColor).trigger('change');
      }
    }
