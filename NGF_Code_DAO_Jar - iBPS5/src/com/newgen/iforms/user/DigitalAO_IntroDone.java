...
public class DigitalAO_IntroDone extends DigitalAO_Common
{
	...
	{
		String strReturn="";
		DigitalAO.mLogger.debug("This is iRBL_IntroDone_Event");
		if("InsertIntoHistory".equals(controlName))
		{
			try {
				DigitalAO.mLogger.debug("Reject Reasons Grid Length is "+data);
				...
				for(int p=0;p<Integer.parseInt(data);p++)
				{
					...
					DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Complete Reject Reasons" + completeReason);
					...
					{						
						if(completeReason.indexOf("-")>-1)
						{
							...
							DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons code" + strRejectCodes);
							strRejectReasons=completeReason.substring(completeReason.indexOf("-")+1);
							DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons" + strRejectReasons);
						}
						else
						{
							strRejectReasons=completeReason;
							DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons else block" + strRejectReasons);
						}
					}	
					else
					{
						DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons 1" + strRejectReasons);						
						if(completeReason.indexOf("-")>-1)
						{
						...
						else
						{
							...
							DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons else block2" + strRejectReasons);
						}
						
						DigitalAO.mLogger.debug("WINAME: "+getWorkitemName(iform)+", WSNAME: "+iform.getActivityName()+", ControlName: "+controlName+", Reject Reasons 2" + strRejectReasons);
					}
				...
				DigitalAO.mLogger.debug("Final reject reasons are "+strRejectReasons);
			   ...
			   // SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");			   
				...
				DigitalAO.mLogger.debug("Decision" +iform.getValue("qDecision"));
				...
				DigitalAO.mLogger.debug("Entry Date Time : "+obj.get("Entry Date Time"));
				jsonArray.add(obj);
