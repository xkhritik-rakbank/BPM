...
//Parameter
//	1. sLocalDate	: Date to be formatted
//	2. sDateFormat	: Format of supplied sLocalDate argument
// Function returns date in database format.
//----------------------------------------------------------------------------------------------------
//Function Name                    : LocalToDB(string,string)
//Date Written (DD/MM/YYYY)        : 17-Oct-2006
//Author                           : Manish K. Agrawal
...
//----------------------------------------------------------------------------------------------------
//Function Name                    : validateKeys(string,string)
//Date Written (DD/MM/YYYY)        : 17-Oct-2006
//Author                           : Manish K. Agrawal
//Input Parameters                 : String,String
//Output Parameters                : String
...
// Reason / Cause (Bug No if Any)	:	Notes field.((RKB-RLS-ALS-CRF-0010/RKB-RLS-ALS-CMA-0095/RKB-RLS-ALS-CRF-0046/RKB-RLS-ALS-CRF-0047/RKB-RLS-ALS-CRF-0048/RKB-RLS-ALS-CRF-0049/RKB-RLS-ALS-CRF-0050/RKB-RLS-ALS-CRF-0051/RKB-RLS-ALS-CRF-0052/RKB-RLS-ALS-CRF-0053/RKB-RLS-ALS-CRF-0054/RKB-RLS-ALS-CRF-0055/RKB-RLS-ALS-CRF-0056/RKB-RLS-ALS-CRF-0057/RKB-RLS-ALS-CRF-0058/RKB-RLS-ALS-CRF-0061/RKB-RLS-ALS-CRF-0062/RKB-RLS-GSR-LCOL-0001/RKB-RLS-GSR-LCOL-0005/RKB-RLS-GSR-LCOL-0006//RKB-RLS-GSR-LCOL-0007/RKB-RLS-GSR-LCOL-0008/RKB-RLS-GSR-LCOL-0009/RKB-RLS-GSR-LCOL-0010/RKB-RLS-GSR-LCOL-0011/RKB-RLS-GSR-LCOL-0013//RKB-RLS-GSR-LCOL-0014/RKB-RLS-LAI-BC-0001/RKB-RLS-PL-PD-0004)
// Change Description				:	Notes field.
//---------------------------------------------------------------------------------------------------->
function validateKeys(e,act){
	
	var re="";
	
//	Field Type				Allowable Characters
//============================================================================
//  Alphanumeric	-		alphabet, Numeric, space, ".", ",", "/", "(", ")", "-", "_", "@", "$", "%"
//  Numeric			-		Numeric, space, "."
//  Exception for fields(OFFICENOEXTN/RESIDENCENO/MOBILE/CONTACTNUMBER/OFFICENO/FAXNO/RESINO/MOBNO/EX/RESNO/MOB/OFFNOEX/OFFICENOEEXTN)						-		Numeric, space, "+"
// Debit Account-	 0-9

	if(e.name.toUpperCase()== 'FUNDINGACCOUNT' || e.name.toUpperCase()== 'DEBITACCOUNT_FORCHARGE' || e.name.toUpperCase()== 'ACCOUNTNO' || e.name.toUpperCase()== 'ACTOCREDIT' || e.name.toUpperCase()== 'DBTACCCHRG' || e.name.toUpperCase()== 'DBTACC' || e.name.toUpperCase()== 'FACFR' || e.name.toUpperCase()== 'FACTO'  || e.name.toUpperCase()== 'GUARANTORACCOUNTNO' || e.name.toUpperCase()== 'EQUATIONMASTER' /*|| e.name.toUpperCase()== 'CREDITCARDNO'*/){
		re = /[^0-9]/g;
		if(re.test(e.value)){
	...
	if (e.name.toUpperCase()=='CREDITCARDNO')
	{
		validateCCNDataOnKeyUp(e)
			return false;
	}
	
	if(e.name.toUpperCase()=='OFFICENOEXTN' || e.name.toUpperCase()=='RESIDENCENO' || e.name.toUpperCase()=='MOBILE' || e.name.toUpperCase()=='CONTACTNUMBER' || e.name.toUpperCase()=='OFFICENO' || e.name.toUpperCase()=='FAXNO'  || e.name.toUpperCase()=='RESINO' || e.name.toUpperCase()=='MOBNO' || e.name.toUpperCase()=='EX' || e.name.toUpperCase()=='RESNO' || e.name.toUpperCase()=='MOB' || e.name.toUpperCase()=='OFFNOEX' || e.name.toUpperCase()=='OFFICENOEEXTN' || e.name.toUpperCase()=='MOBILENO' || e.name.toUpperCase()=='LANDLINENO'){
		re = /[^0-9+\s]/g;
		if(re.test(e.value)){
			//alert("Only numbers, +, space are allowed.");
			e.value = e.value.replace(re, "");		
		}
		return;
	}
	
     if(e.name.toUpperCase()=='DEBITACCOUNT' || e.name.toUpperCase()=='GuarantorAccountNo'){
		re = /[^0-9-]/g;
		if(re.test(e.value)){
			//alert("Only numbers are allowed.");
			e.value = e.value.replace(re, "");
		...
		/*if (e.value.length == 4 || e.value.length == 11){
	      e.value = e.value + '-';
	      }*/
...
//		if (act=='digitsOnly')
//			alert("Only numbers are allowed.");
		if (act=='dashdigitsOnly')
			alert("Only numbers and - are allowed.");
		if (act=='alphanumeric')
			alert("Only alphabets, numbers, space, ., /, (, ), -, _, @, $, % and , are allowed.");
...
//----------------------------------------------------------------------------------------------------
//Function Name                    : validateKeys_OnBlur(string,string)
//Date Written (DD/MM/YYYY)        : 17-Oct-2006
//Author                           : Manish K. Agrawal
//Input Parameters                 : String,String
//Output Parameters                : String
...
//----------------------------------------------------------------------------------------------------
// Changed By						:	Manish K. Agrawal
// Reason / Cause (Bug No if Any)	:	Format.((RKB-RLS-ALS-CRF-0004/RKB-RLS-ALS-CRF-0044/RKB-RLS-ALS-CRF-0045/RKB-RLS-PL-ES-0006)
// Change Description				:	Format.
//---------------------------------------------------------------------------------------------------->
function validateKeys_OnBlur(e,act,iInt,iDec){
	if(e.name.toUpperCase()== 'FUNDINGACCOUNT'  || e.name.toUpperCase()== 'DEBITACCOUNT_FORCHARGE' || e.name.toUpperCase()== 'ACCOUNTNO' || e.name.toUpperCase()== 'ACTOCREDIT' || e.name.toUpperCase()== 'DBTACCCHRG' || e.name.toUpperCase()== 'DBTACC' || e.name.toUpperCase()== 'FACFR' || e.name.toUpperCase()== 'FACTO'  || e.name.toUpperCase()== 'GUARANTORACCOUNTNO' || e.name.toUpperCase()== 'EQUATIONMASTER') {
		re = /[^0-9]/g;
		if(re.test(e.value)){
			alert("Only numbers are allowed....");
			e.value = e.value.replace(re, "");		
	...
	if(e.name.toUpperCase()=='OFFICENOEXTN' || e.name.toUpperCase()=='RESIDENCENO' || e.name.toUpperCase()=='MOBILE' || e.name.toUpperCase()=='CONTACTNUMBER' || e.name.toUpperCase()=='OFFICENO' || e.name.toUpperCase()=='FAXNO'  || e.name.toUpperCase()=='RESINO' || e.name.toUpperCase()=='MOBNO' || e.name.toUpperCase()=='EX' || e.name.toUpperCase()=='RESNO' || e.name.toUpperCase()=='MOB' || e.name.toUpperCase()=='OFFNOEX' || e.name.toUpperCase()=='OFFICENOEEXTN' || e.name.toUpperCase()=='MOBILENO' || e.name.toUpperCase()=='LANDLINENO'){
		re = /[^0-9+\s]/g;
		if(re.test(e.value)){
			alert("Only numbers, + and space are allowed.");
			e.value = e.value.replace(re, "");		
		}
         
		if (e.name.toUpperCase()=='LANDLINENO')     // rakbank changes     06/07/2007
         {
			   var l = e.value.length-7;
			   var m = e.value.indexOf('+');
               //alert(m);
			   //alert(l);
			   if (l>m)
			   {
				   document.forms.dataform.q_landlineno.value = e.value.substring(l);
				   document.forms.dataform.LANDLINENP.value = e.value.substring(l);
			   }
			   else
			   {
                   document.forms.dataform.q_landlineno.value = e.value.substring(m+1);
				   document.forms.dataform.LANDLINENP.value = e.value.substring(m+1);
			   }

			   
         }
        if (e.name.toUpperCase()=='MOBILENO')
         {
			   var l = e.value.length-7;
			   var m = e.value.indexOf('+');
			   if (l>m)
			   { 
			        document.forms.dataform.q_mobileno.value = e.value.substring(l);
					document.forms.dataform.MOBILENP.value = e.value.substring(l);
               }
			   else
			   {
				   document.forms.dataform.q_mobileno.value = e.value.substring(m+1);
				   document.forms.dataform.MOBILENP.value = e.value.substring(m+1);
			   }
		 }
	...
	if(e.name.toUpperCase()=='DEBITACCOUNT'){
		re = /[^0-9-]/g;
		if(re.test(e.value)){
			alert("Only numbers and - are allowed.");
			e.value = e.value.replace(re, "");
	...
	if(act=="float"){		
		...
		if(m!=-1){//multiple '.'
			alert("Invalid value has entered. The format should be " + (iInt+iDec) + "," + iDec + ".");
			e.value='';
			e.focus();
		}
		//----------------------------------------------------------------------------------------------------
		// Changed By						:	Manish K. Agrawal
		// Reason / Cause (Bug No if Any)	:	Format.(RKB-RLS-GSR-LCOL-0012/RKB-RLS-PL-IHR-0004)
		// Change Description				:	Format.
		//---------------------------------------------------------------------------------------------------->
		...
		if(i!=-1){
			...
			//----------------------------------------------------------------------------------------------------
			// Changed By						:	Manish K. Agrawal
			// Reason / Cause (Bug No if Any)	:	Format.((RKB-RLS-GSR-LCOL-0004)
			// Change Description				:	Format.
			//---------------------------------------------------------------------------------------------------->
			...
			if(parseInt(iInt)<e.value.length){
				alert("Invalid value has entered. The format should be " + (iInt+iDec) + "," + iDec + ".");
				e.value='';
				e.focus();
			}
...
//		case "digitsOnly":re = /[^0-9]/g;break;
	...
	/*if((act=="numeric" || act=="float") && Trim(e.value)==''){
		e.value=0;
	}*/
...
//alert(document.forms.dataform.ProcessCode.value);
//alert(e.name);

	if (document.forms.dataform.ProcessCode.value=="PL_PD" || document.forms.dataform.ProcessCode.value=="GSR_EmpAdd" || document.forms.dataform.ProcessCode.value=="PL_ES" || document.forms.dataform.ProcessCode.value=="PL_IPDR" || document.forms.dataform.ProcessCode.value=="PL_PS" || document.forms.dataform.ProcessCode.value=="GSR_LCOL")
	{
		
	   if(!((document.forms.dataform.ProcessCode.value=="PL_PS" ||
		document.forms.dataform.ProcessCode.value=="GSR_LCOL" ||   
	   document.forms.dataform.ProcessCode.value=="PL_ES" || document.forms.dataform.ProcessCode.value=="PL_PD")  && (e.name=='ChargesReceived' || e.name=='CHARGESRECEIVED1' ||
		   e.name=='AMOUNT1' || e.name=='Amount' || e.name=='ChargesRecvd' || e.name=='CHARGESRECVD1') && e.value<=0))
		{
	    e.value = e.value.replace(/^[0]+/g,""); 
		dollarAmount(e);     //rakbank change 4 jul 2007
		//alert(e.value);
	}
	}
    }
...
function validateCCN(cntrl){
	if (cntrl.name.toUpperCase()=='CREDITCARDNO')
	{
	...
	//ssssss
	if(cntrl.name.toUpperCase()=='DEBITACCOUNT' || cntrl.name.toUpperCase()=='GuarantorAccountNo'){
	var keycode=event.keyCode;
	var cntrlValue=cntrl.value;
    if(keycode!=46&&keycode!=8&&(cntrlValue.length==4||cntrlValue.length==11))
		cntrl.value=cntrlValue+"-";
	}
