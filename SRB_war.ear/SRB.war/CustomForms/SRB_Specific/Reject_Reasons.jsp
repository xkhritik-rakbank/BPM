...
<%
	...
	String RejectReasons = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1!=null?input1:"");
	...
	String WINAME = ESAPI.encoder().encodeForSQL(new OracleCodec(), input3!=null?input3:"");
	...
	String username = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4!=null?input4:"");
...
<html>
	<head>
		...
		<link rel="stylesheet" href="\SRB\webtop\en_us\css\docstyle.css">
		<link rel="stylesheet" href="\SRB\webtop\en_us\css\jquery.autocomplete.css">
				...
				if(data!=null && data!="" && data!='{}'){
					...
					for(var i=0 ; i< arrACTFields.length ; i++){
						...
						$(document).ready(function(){
							$("#"+temp[0]).autocomplete(values,{max:0,minChars:0,matchContains:true});							
						});
					}		
				}
			}
...
//***********************************************************************************//			
		...
		//if(WSNAME=='Q4')//Q4==OPS Checker
			//{
				var RejectReasonsListmaker = document.getElementById('RejectReasonsList');
				RejectReasonsListmaker.value = '<%=RejectReasons%>';
	...
	<body class="EWGeneralRB" style="border: 2px solid #b20000; border-style:outset;" bgcolor="#FFFBF0" onload="setAutocompleteData();opsmakerrejectreason();populateListbox('<%=RejectReasons%>');" onUnload="CancelClick(1);">		
...
<%
	...
	String workstename = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5!=null?input5:"");
	//added by shamily to get workstepname
	if (workstename != null) {workstename=workstename.replace("'","''");}
	if(workstename.equalsIgnoreCase("Print Dispatch"))
		workstename = "Print and Dispatch";
	
	Query="select item_code,Item_Desc from usr_0_srb_error_desc_Master with(nolock) where isactive='Y' AND workstename=:workstename order by id ASC";
