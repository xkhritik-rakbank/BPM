...
//***********************************************************************************//			
			function OKClick()
			{
			   ...
			   var username = '<%=username%>';
				...
				//Updating external table for Email to compliance
				var requestType = "UPDATE_REJECT_REASONS";
				var splCharacter = '&';
				...
				var url = "/webdesktop/CustomForms/TT_Specific/HandleAjaxUpdate.jsp?Wsname="+Wsname+"&Winame="+Winame+"&username="+username+"&requestType="+requestType+"&Rejectreason="+encodeURIComponent(rejectreason);
				...
				{
					alert("Some error while updating the rejection status");
				}
...
<%
	...
	Query="select item_code,Item_Desc from USR_0_TT_Error_Desc_Master with(nolock) where isactive='Y' AND workstename='"+WSNAME+"' order by id ASC";
	WriteLog("Query for getting reject reasons:"+Query);
	
	inputXML = "<?xml version='1.0'?><APSelectWithColumnNames_Input><Option>APSelectWithColumnNames</Option><Query>" + Query + "</Query><EngineName>" + wfsession.getEngineName() + "</EngineName><SessionId>" + wfsession.getSessionId() + "</SessionId></APSelectWithColumnNames_Input>";
