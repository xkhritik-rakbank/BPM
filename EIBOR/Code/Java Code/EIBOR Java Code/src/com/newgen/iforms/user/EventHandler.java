...
public class EventHandler extends EIBOR_Common  implements IFormServerEventHandler
{	
	...
	{	
		...
		EIBOR.mLogger.info("Inside executeServerEvent() ak 101 ---control: " + control + "\nevent: " + event + "\nStringData: "
				+ Stringdata);
		...
		if("click".equalsIgnoreCase(event))
		{
			...
			try {
				return new EIBOR_Click().clickEvent(iformObj,control,Stringdata);
		...
		{
			return new EIBOR_FormLoad().formLoad(iformObj,control,event,Stringdata);
		}
		else if("introducedone".equalsIgnoreCase(event))
		{
			return new EIBOR_IntroDone().onIntroduceDone(iformObj,control,event,Stringdata);
		}
		else if("Change".equalsIgnoreCase(event))
		{
			return new EIBOR_Change().changeEvent(iformObj,control,event,Stringdata);
		}
