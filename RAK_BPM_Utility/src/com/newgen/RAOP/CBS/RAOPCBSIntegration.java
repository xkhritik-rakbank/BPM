...
public class RAOPCBSIntegration {
			...
			HashMap<String, String> socketDetailsMap,String docDownloadPath,String volumeId,String siteId, String Sig_Remarks) throws Exception
	{
		...
		if (sMainCode.equals("0") && sTotalRecords > 0)
		{
			...
			if(!"Y".equalsIgnoreCase(strIsExistingCustomer) && !"Success".equalsIgnoreCase(strCREATECIFSTATUS) && ("".equalsIgnoreCase(strCIFNumber) || "null".equalsIgnoreCase(strCIFNumber)))
			{
				...
				// Start - Minor customer changes CR-140 implemented on 04022022
				String RelationshipDetailsForMinor = "";
				String MinorFlag = "N";
				long diffInYears = CommonMethods.findDifference(strDOB,"yyyy-MM-dd","Years");
				RAOPCBSLog.RAOPCBSLogger.debug("diffInYears: "+diffInYears);
        		if(diffInYears < 21) // less than 21 considered as minor
        		{
        			RelationshipDetailsForMinor = "<RelationshipDetails>"
							+ "<ChildCIFID>7777777</ChildCIFID>\n"
							+ "<Relationship>Guardian</Relationship>\n"
							+ "<RelationshipCategory>Social</RelationshipCategory>\n"
							+ "<GuardCode>LEGAL GUARDIAN</GuardCode>\n"
							+ "<ChildEntity>CUSTOMER</ChildEntity>\n"
							+ "<ChildEntityType>Retail</ChildEntityType>\n"
						+ "</RelationshipDetails>";
        			MinorFlag = "Y";
        		}
				// End - Minor customer changes CR-140 implemented on 04022022
				...
				String PersonDetails=personDetailsForCreateCustomer(strTitle, strFirstName, strMiddleName, strLastName, ShortName, strFirstName+" "+strMiddleName+" "+strLastName, strGender, "N", strResAddrCountry, strMaritalStatus, strNationality, strDOB, MinorFlag);
			...
			//Start - Added for Finacle update
			
			if("Success".equalsIgnoreCase(objResponseBean.getCifCreationReturnCode()) && !"Success".equalsIgnoreCase(strUPDATECIFSTATUS) && "Y".equalsIgnoreCase(strIsExistingCustomer))
			{
				String finacleQueryString = "SELECT TOP 1 CHECK_TITLE,CHECK_FIRST_NAME,CHECK_MIDDLE_NAME,CHECK_LAST_NAME,"
						+"CHECK_EMIRATES_ID,CHECK_EMIRATES_ID_EXPIRY_DATE, CHECK_VISA_ISSUE_DATE,"
						+"CHECK_VISA_EXPIRY_DATE, CHECK_RISK_SCORE, CHECK_PASSPORT_NUMBER,"
						+"CHECK_PASSPORT_EXPIRY_DATE FROM RB_RAOP_TXNTABLE with (nolock) where WI_NAME='"+wi_name+"'";
				
				RAOPCBSLog.RAOPCBSLogger.debug("finacle Query:"+finacleQueryString);
				try 
				{
					String sFinacleInputXML =CommonMethods.apSelectWithColumnNames(finacleQueryString, cabinetName, sessionID);
					RAOPCBSLog.RAOPCBSLogger.debug("Input XML for Apselect from FINACLE Extrenal Table "+sFinacleInputXML);

					String sFinacleOutputXML=RAOPCBS.WFNGExecute(sFinacleInputXML, sJtsIp, sJtsPort,1);
					RAOPCBSLog.RAOPCBSLogger.debug("Output XML for Finacle extranl Table select "+sFinacleOutputXML);

					XMLParser sFinacleXMLParser= new XMLParser(sFinacleOutputXML);
					String sFinacleMainCode = sFinacleXMLParser.getValueOf("MainCode");
					RAOPCBSLog.RAOPCBSLogger.debug("SFinacleMainCode: "+sFinacleMainCode);

					int sFinacleTotalRecords = Integer.parseInt(sFinacleXMLParser.getValueOf("TotalRetrieved"));
					RAOPCBSLog.RAOPCBSLogger.debug("SFinacleTotalRecords: "+sFinacleTotalRecords);
					
					if (sFinacleMainCode.equals("0") && sFinacleTotalRecords > 0)
					{
						RAOPCBSLog.RAOPCBSLogger.debug("Coming in If Sajan");
						//CustomerBean objCustBean1=new CustomerBean();
						String strChkTitle="";
						strChkTitle=sFinacleXMLParser.getValueOf("CHECK_TITLE");
						objCustBean.setChkTitle(strChkTitle);
						RAOPCBSLog.RAOPCBSLogger.debug("TITLE is "+strChkTitle);
						
						String strChk_First_Name="";
						strChk_First_Name=sFinacleXMLParser.getValueOf("CHECK_FIRST_NAME").trim();
						objCustBean.setChkFirstName(strChk_First_Name);
						RAOPCBSLog.RAOPCBSLogger.debug("First Name is "+strChk_First_Name);
						
						String strchkMiddleName="";
						strchkMiddleName=sFinacleXMLParser.getValueOf("CHECK_MIDDLE_NAME").trim();
						objCustBean.setChkMiddleName(strchkMiddleName);
						RAOPCBSLog.RAOPCBSLogger.debug("MIDDLE_NAME is "+strchkMiddleName);
						
						String strchkLastName="";
						strchkLastName=sFinacleXMLParser.getValueOf("CHECK_LAST_NAME").trim();
						objCustBean.setChkLastName(strchkLastName);
						RAOPCBSLog.RAOPCBSLogger.debug("LAST_NAME is "+strchkLastName);
						
						String strchkEmiratesId="";
						strchkEmiratesId=sFinacleXMLParser.getValueOf("CHECK_EMIRATES_ID").trim();
						objCustBean.setChkEmiratesID(strchkEmiratesId);
						RAOPCBSLog.RAOPCBSLogger.debug("EMIRATES_ID is "+strchkEmiratesId);
						
						String strchkEmiratesExpiryDate="";
						strchkEmiratesExpiryDate=sFinacleXMLParser.getValueOf("CHECK_EMIRATES_ID_EXPIRY_DATE").trim();
						objCustBean.setChkEmiratesExpiryDate(strchkEmiratesExpiryDate);
						RAOPCBSLog.RAOPCBSLogger.debug("EMIRATES_EXPIRY_DATE is "+strchkEmiratesExpiryDate);
						
						String strchkVisaIssueDate="";
						strchkVisaIssueDate=sFinacleXMLParser.getValueOf("CHECK_VISA_ISSUE_DATE").trim();
						objCustBean.setChkVisaIssueDate(strchkVisaIssueDate);
						RAOPCBSLog.RAOPCBSLogger.debug("CHECK_VISA_ISSUE_DATE is "+strchkVisaIssueDate);
						
						String strchkVisaExpiryDate="";
						strchkVisaExpiryDate=sFinacleXMLParser.getValueOf("CHECK_VISA_EXPIRY_DATE").trim();
						objCustBean.setChkVisaExpiryDate(strchkVisaExpiryDate);
						RAOPCBSLog.RAOPCBSLogger.debug("CHECK_VISA_EXPIRY_DATE is "+strchkVisaExpiryDate);
						
						String strchkRiskScore="";
						strchkRiskScore=sFinacleXMLParser.getValueOf("CHECK_RISK_SCORE").trim();
						objCustBean.setChkRiskScore(strchkRiskScore);
						RAOPCBSLog.RAOPCBSLogger.debug("CHECK_RISK_SCORE is "+strchkRiskScore);
						
						String strchkPassNo="";
						strchkPassNo=sFinacleXMLParser.getValueOf("CHECK_PASSPORT_NUMBER").trim();
						objCustBean.setChkPassNo(strchkPassNo);
						RAOPCBSLog.RAOPCBSLogger.debug("CHECK_PASSPORT_NUMBER is "+strchkPassNo);
						
						String strchkPassExpiryDate="";
						strchkPassExpiryDate=sFinacleXMLParser.getValueOf("CHECK_PASSPORT_EXPIRY_DATE").trim();
						objCustBean.setChkPassExpiryDate(strchkPassExpiryDate);
						RAOPCBSLog.RAOPCBSLogger.debug("CHECK_PASSPORT_EXPIRY_DATE is "+strchkPassExpiryDate);
						
						
						String getstrChkTitle=objCustBean.getChkTitle();
						RAOPCBSLog.RAOPCBSLogger.debug(getstrChkTitle);
						String getstrChk_First_Name=objCustBean.getChkFirstName();
						RAOPCBSLog.RAOPCBSLogger.debug(getstrChk_First_Name);
						String getstrchkMiddleName=objCustBean.getChkMiddleName();
						RAOPCBSLog.RAOPCBSLogger.debug(getstrchkMiddleName);
						String getstrchkLastName=objCustBean.getChkLastName();
						String getstrchkEmiratesId=objCustBean.getChkEmiratesId();
						RAOPCBSLog.RAOPCBSLogger.debug(getstrchkEmiratesId);
						String getstrchkEmiratesExpiryDate=objCustBean.getChkEmiratesExpiryDate();
						String getstrchkVisaIssueDate=objCustBean.getChkVisaIssueDate();
						String getstrchkVisaExpiryDate=objCustBean.getChkVisaExpiryDate();
						String getstrchkRiskScore=objCustBean.getChkRiskScore();
						String getstrchkPassNo=objCustBean.getChkPassNo();
						String getstrchkPassExpiryDate=objCustBean.getChkPassExpiryDate();
						
						if(getstrChkTitle.equalsIgnoreCase("true") || getstrChk_First_Name.equalsIgnoreCase("true") 
								|| getstrchkMiddleName.equalsIgnoreCase("true")
								|| getstrchkLastName.equalsIgnoreCase("true") || getstrchkEmiratesId.equalsIgnoreCase("true") 
								|| getstrchkEmiratesExpiryDate.equalsIgnoreCase("true") || getstrchkVisaIssueDate.equalsIgnoreCase("true")
								|| getstrchkVisaExpiryDate.equalsIgnoreCase("true") 
								|| getstrchkPassNo.equalsIgnoreCase("true")
								|| getstrchkPassExpiryDate.equalsIgnoreCase("true")
								|| getstrchkRiskScore.equalsIgnoreCase("true"))
						{
							
							String RtlAddnlDetails = "";
							if(getstrChkTitle.equalsIgnoreCase("true") || getstrChk_First_Name.equalsIgnoreCase("true") 
									|| getstrchkMiddleName.equalsIgnoreCase("true")
									|| getstrchkLastName.equalsIgnoreCase("true") || getstrchkRiskScore.equalsIgnoreCase("true"))
							{
								RtlAddnlDetails=personDetailsForFinacleUpdate(objCustBean,strTitle, strFirstName, strMiddleName, strLastName,strRiskScore);
								RAOPCBSLog.RAOPCBSLogger.debug("RtlAddnlDetails :"+RtlAddnlDetails);
							}
							
							String VisaDocType = "";
							if(getstrchkVisaIssueDate.equalsIgnoreCase("true") ||getstrchkVisaExpiryDate.equalsIgnoreCase("true"))
								VisaDocType=DocDetailsForUpdateCustomer("VISA",objCustBean);
							
							String EmiratesDocType = "";	
							if (getstrchkEmiratesId.equalsIgnoreCase("true") || getstrchkEmiratesExpiryDate.equalsIgnoreCase("true"))
								EmiratesDocType=DocDetailsForUpdateCustomer("EMID",objCustBean);
							
							String Passport = "";
							if (getstrchkPassNo.equalsIgnoreCase("true") ||getstrchkPassExpiryDate.equalsIgnoreCase("true"))
								Passport=DocDetailsForUpdateCustomer("PPT",objCustBean);				
							...
							{
								RAOPCBSLog.RAOPCBSLogger.error("exception caught in getting inputxml customer update for CIF "+strCIFNumber+" "+ e.getMessage());
								e.printStackTrace();
							}
							...
							RAOPCBSLog.RAOPCBSLogger.error("No existing checkbox details are available for update: "+strCIFNumber);
							objResponseBean.setCifUpdateReturnCode("Success");
							objResponseBean.setIntCallFailed(" ");
							objResponseBean.setIntFailedReason(" ");
							objResponseBean.setIntFailedCode(" ");
						...
						RAOPCBSLog.RAOPCBSLogger.error("No existing details are available for update: "+strCIFNumber);
						objResponseBean.setCifUpdateReturnCode("Success");
						objResponseBean.setIntCallFailed(" ");
						objResponseBean.setIntFailedReason(" ");
						objResponseBean.setIntFailedCode(" ");
				...
				catch(Exception e)
				{
					...
					RAOPCBSLog.RAOPCBSLogger.error("Exception occurred in outputResponseXML in exsiting cif update" + e.getStackTrace());
					objResponseBean.setCifUpdateReturnCode("Failure");
					objResponseBean.setIntegrationDecision("Failure");
					objResponseBean.setIntCallFailed("CUSTOMER_UPDATE_REQ");
					return objResponseBean;
	...
	public String personDetailsForCreateCustomer(String TitlePrefix, String FirstName, String MiddleName, String LastName, String ShortName, String FullName, String Gender, String NonResidentFlag, String ResCountry, String MaritalStatus, String Nationality, String DateOfBirth, String MinorFlag)
	{
		String PersonalXml = "<PersonDetails><TitlePrefix></TitlePrefix><FirstName></FirstName><MiddleName></MiddleName><LastName></LastName><ShortName></ShortName><FullName></FullName><Gender></Gender><MinorFlag>"+MinorFlag+"</MinorFlag><NonResidentFlag></NonResidentFlag><ResCountry></ResCountry><MaritalStatus></MaritalStatus><Nationality></Nationality><DateOfBirth></DateOfBirth></PersonDetails>";
	...
	public String personDetailsForFinacleUpdate(CustomerBean objCustBean, String TitlePrefix, String FirstName, String MiddleName, String LastName, String RiskScore)
	{
		String PersonalXml = "<RtlAddnlDet><Title></Title><FirstName></FirstName><MidName></MidName><LastName></LastName><FullName></FullName><RiskProfile></RiskProfile></RtlAddnlDet>";

		StringBuffer PersonalB = new StringBuffer(PersonalXml);
		if("true".equalsIgnoreCase(objCustBean.getChkTitle().trim()) 
				&& !TitlePrefix.equalsIgnoreCase("") && TitlePrefix!=null && !TitlePrefix.equals(""))
			PersonalB = PersonalB.insert(PersonalB.indexOf("<Title>")+"<Title>".length(),TitlePrefix );
		else
			PersonalB = PersonalB.delete(PersonalB.indexOf("<Title>"), PersonalB.indexOf("</Title>")+"</Title>".length());

		if ("true".equalsIgnoreCase(objCustBean.getChkFirstName().trim()) || "true".equalsIgnoreCase(objCustBean.getChkMiddleName().trim()) || "true".equalsIgnoreCase(objCustBean.getChkLastName().trim()) )
		{		
			if(!FirstName.equalsIgnoreCase("") && FirstName!=null && !FirstName.equals("") )
				PersonalB = PersonalB.insert(PersonalB.indexOf("<FirstName>")+"<FirstName>".length(),FirstName );
			else
			...
			if(!MiddleName.equalsIgnoreCase("") && MiddleName!=null && !MiddleName.equals("") )
				PersonalB = PersonalB.insert(PersonalB.indexOf("<MidName>")+"<MidName>".length(),MiddleName );
			else
				PersonalB = PersonalB.delete(PersonalB.indexOf("<MidName>"), PersonalB.indexOf("</MidName>")+"</MidName>".length());
			...
			String sFullName = "";
			if(!MiddleName.equalsIgnoreCase("") && MiddleName!=null && !MiddleName.equals("") )
				sFullName = FirstName + " " + MiddleName + " " + LastName;
			else
				sFullName = FirstName  + " " + LastName;
			
			PersonalB = PersonalB.insert(PersonalB.indexOf("<FullName>")+"<FullName>".length(),sFullName );
		}
		else {
			PersonalB = PersonalB.delete(PersonalB.indexOf("<FirstName>"), PersonalB.indexOf("</FirstName>")+"</FirstName>".length());
			PersonalB = PersonalB.delete(PersonalB.indexOf("<MidName>"), PersonalB.indexOf("</MidName>")+"</MidName>".length());
		...
		if("true".equalsIgnoreCase(objCustBean.getChkRiskScore().trim()) &&
				!RiskScore.equalsIgnoreCase("") && RiskScore!=null && !RiskScore.equals("") )
			PersonalB = PersonalB.insert(PersonalB.indexOf("<RiskProfile>")+"<RiskProfile>".length(),RiskScore );
		else
			PersonalB = PersonalB.delete(PersonalB.indexOf("<RiskProfile>"), PersonalB.indexOf("</RiskProfile>")+"</RiskProfile>".length());
	...
	public String OECDDetailsForUpdateCustomer(CustomerBean objCustDataBean,String sessionId,String cabinetName,String jtsIp,String jtsPort)
	{
		...
		{
			String OecdTaxXml="<ReporCntryDet><CntryOfTaxRes></CntryOfTaxRes><TINNumber></TINNumber><NoTINReason></NoTINReason><NoTINReasonDetails></NoTINReasonDetails></ReporCntryDet>";
			...
			if(objArrayOecdTaxList.get(i).getRemarks() !=null && !objArrayOecdTaxList.get(i).getRemarks().equals("") && "REASON B".equalsIgnoreCase(objArrayOecdTaxList.get(i).getNoTinReason().trim()))
				OecdTaxB = OecdTaxB.insert(OecdTaxB.indexOf("<NoTINReasonDetails>")+"<NoTINReasonDetails>".length(),objArrayOecdTaxList.get(i).getRemarks());
			else
				OecdTaxB = OecdTaxB.delete(OecdTaxB.indexOf("<NoTINReasonDetails>"), OecdTaxB.indexOf("</NoTINReasonDetails>")+"</NoTINReasonDetails>".length());
	...
	public String DocDetailsForUpdateCustomer(String DocType,CustomerBean objCustBean)
	{
			...
			{
				expdate=objCustBean.getVisaExpDate().trim();
				DocNumber=objCustBean.getVisaNumber().trim();	
				DocIssDate=objCustBean.getVisaIssDate().trim();
			}
			...
			if(!"".equalsIgnoreCase(DocIssDate))
				docTypeXml = docTypeXml +"<DocIssDate>"+DocIssDate+"</DocIssDate>\n";
			
			docTypeXml = docTypeXml+"<DocExpDate>"+expdate+"</DocExpDate>\n" +
			"</DocDet>";
