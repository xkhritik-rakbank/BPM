	...
	private void startRAOPDocumentUtility()
	{
		...
		if (wiList != null)
		{
			for (WorkItem wi : wiList)
			{
				...
				String ISDOCPROVIDED = wi.getAttribute("ISDOCPROVIDED");
								...
								if(foldername.equalsIgnoreCase(workItemName))
								{
									...
									for (File listOfDoc : listOfDocument)
									{
										if (listOfDoc.isFile())
										{
											...
											else if (strDocumentName.contains("Company_Ownership"))
												DocNameAsProcess = "Company_Ownership";
											else
											...
											for (int i = 0; i < 3; i++)
											{
												...
												if(lLngFileSize != 0L)
												{
													...
													try
													{
														...
														if("Y".equalsIgnoreCase(ISDOCPROVIDED.trim()))
														{
															ISDOCPROVIDED = "N"; // setting flag as N when at least one document is attached - 14122021
															RAOPDocumentLog.RAOPDocumentLogger.info("workItemName: "+workItemName+" Atlease one doc is attached");
															updateExternalTable(ExternalTable,"ISDOCPROVIDED","'"+ISDOCPROVIDED+"'","ITEMINDEX='"+parentFolderIndex+"'");
														}
													}
				...
				try
				{
					if (ErrorFlag)
					{
						...
						// fourth - if none of doc received after additional information required
						// fifth - Success - all docs including mandatory are attached - same as above
						...
						//4. if none of doc received after additional information required
						else if("Y".equalsIgnoreCase(ISDOCPROVIDED.trim()))
						{
							RAOPDocumentLog.RAOPDocumentLogger.info("Inside case 4, if none of doc received after additional information required");
							decisionToUpdate = "Failure";
							FailedIntegration= "DocNotAvailable";
							ErrorMessageFrmIntegration = "Document Not Available";
							Integration_error_received="Attach_Cust_Doc";
							...
							if(iNoOfTries == 4 || iNoOfTries == 8 || iNoOfTries == 12)
								sendMailTrigger(workItemName);
							//## Mail trigger - added by Vaishvi - end
							RAOPDocumentLog.RAOPDocumentLogger.info("Case 4 - iNoOfTries == "+iNoOfTries+" iMaxNoOfTries == "+iMaxNoOfTries);
							if (iNoOfTries > iMaxNoOfTries)
							{
								historyCaller(workItemName,false);
								doneWorkItem(workItemName, "");
						...
						{
							RAOPDocumentLog.RAOPDocumentLogger.info("Case 5, i.e, Success case, all mandatory documents attached");
							historyCaller(workItemName,true);
							decisionToUpdate="Success";
							FailedIntegration=" ";
							ErrorMessageFrmIntegration=" ";
		 ...
		 private List loadWorkItems(String queueID,String sessionId) throws NumberFormatException, IOException, Exception
		{
		 ...
		 public static String get_timestamp()
			...
			private boolean sendMailTrigger(String WIName)
			{
				...
				MAILMESSAGE=raopDocumentCofigParamMap.get("MailMessage").replace("$workItemNumber$", WIName);
			...
			public String checkMandateDoc(String employmentType){
				...
				if (!docListXML.trim().equalsIgnoreCase("F"))
				{
					...
					// considering FATCA-CRS available on the workitem where OECD doctype exists added on 23112021
					/*if(listOfAllDocs.contains("OECD")){
						listOfAllDocs.add("FATCA-CRS");
					}*/
					//*******************
					...
					if("1".equals(isRiskRaised) && "Salaried".equalsIgnoreCase(employmentType)){
					...
					else if("1".equals(isRiskRaised) && "Self employed".equalsIgnoreCase(employmentType)){
						sqlQuery = "select distinct (DocType) from USR_0_RAOP_MandatoryDocMapping with(nolock) where applicability IN ('Common','EMP-Self-Employed') and IsActive = 'Y'";
						addDocForHighRisk = true;
					}
					else if("1".equals(isRiskRaised) && "A".equalsIgnoreCase(employmentType)){
						sqlQuery = "select distinct (DocType) from USR_0_RAOP_MandatoryDocMapping with(nolock) where applicability IN ('Common','EMP-Salaried & Self-Employed') and IsActive = 'Y'";
						addDocForHighRisk = true;
					}
					else if("1".equals(isRiskRaised)){
