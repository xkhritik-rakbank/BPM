...
String input1 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("ReasonCodes"), 1000, true) );
	...
	logger.info("reject reasons code is "+RejectReasons);
...
<%
	...
	String subXML="";	
	String workstename = workstepName_Esapi;  //added by shamily to get workstepname
	if (workstename != null) {workstename=workstename.replace("'","");}
	
	if (workstename.equalsIgnoreCase("AU_Officer1"))
		workstename="AU_Officer";
	if (workstename.equalsIgnoreCase("CBRB_Maker_Additional"))
		workstename="CBRB_Maker";
	if (workstename.equalsIgnoreCase("Quality_Control_Additinal"))
		workstename="Quality_Control";
	if (workstename.equalsIgnoreCase("AECB_Additional"))
		workstename="AECB";
	if (workstename.equalsIgnoreCase("CROPS_Offer_Letter"))
		workstename="CROPS_DocsChecker";
	if (workstename.equalsIgnoreCase("AttachAdditionalDocs"))
		workstename="Business_Rejects";	
	
	Query="select item_code,Item_Desc from usr_0_RBL_error_desc_Master with(nolock) where isactive='Y' AND workstename=:workstename order by id ASC";
	params = "workstename=="+workstename;    //modified by shamily to get workstepname
	logger.info("Query for getting reject reasons"+Query);
