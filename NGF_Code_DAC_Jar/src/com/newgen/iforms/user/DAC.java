...
public class DAC implements IFormListenerFactory {

	private static String loggerName = "DACLogger";
	private static String processName = "DAC";
	...
	public static void setLogger() {
		try {
			...
			if (loggerDateMap.get(logDateFormat.format(date)) != null
					&& loggerDateMap.get(logDateFormat.format(date)).equalsIgnoreCase("Y")) {
			...
			p.load(new FileInputStream(System.getProperty("user.dir") + File.separator + "CustomConfig" + File.separator
					+ "log4j_rak_bpm.properties"));
			String dynamicLog = null;
			String orgFileName = null;
			File d = null;
			File fl = null;

			dynamicLog = "CustomLog/" + processName + "_Logs/" + logDateFormat.format(date) + "/" + processName
					+ "_Log.xml";
