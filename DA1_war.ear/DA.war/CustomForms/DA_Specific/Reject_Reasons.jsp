...
<script language="javascript" src="/DA/webtop/scripts/jquery-latest.js"></script>
<script language="javascript" src="/DA/webtop/scripts/jquery.autocomplete.js"></script>
...
<%
			...
			String input1 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("ReasonCodes", request.getParameter("ReasonCodes"), 1000, true) );
			String ReasonCodes = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1!=null?input1:"");
			WriteLog("Integration jsp: ReasonCodes: "+ReasonCodes);
			...
			String WINAMEEsapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input3!=null?input3:"");
			WriteLog("Integration jsp: wi_name: "+WINAMEEsapi);
			...
			String usernameEsapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4!=null?input4:"");
			WriteLog("Integration jsp: username: "+usernameEsapi);
	...
	String RejectReasons=ReasonCodes.replace("'","''");	
	String WSNAME=WSNAMEEsapi.replace("'","''");	
	String WSNAMER=WSNAME.replace(" ","_");
	String WINAME=WINAMEEsapi.replace("'","''");
    String username=usernameEsapi.replace("'","''"); 	
	String params = "";
%>
<html>
	<head>
		<title>Reject Reasons</title>
		<link rel="stylesheet" href="\DA\webtop\en_us\css\docstyle.css">
		<link rel="stylesheet" href="\DA\webtop\en_us\css\jquery.autocomplete.css">
...
//***********************************************************************************//			
	...
	<table width="100%" border="1" >		
		<tr><td width=100% align=right valign=center><img src="\DA\webtop\images\rak-logo.gif" style="margin-top: 15px;"></td></tr>
	</table>
...
<%
			...
			String workstepNameEsapi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5!=null?input5:"");
			WriteLog("Integration jsp: workstepName: "+workstepNameEsapi);
	...
	String subXML="";	
	String workstename = workstepNameEsapi.replace("'","''");  //added by shamily to get workstepname	

	Query="select item_code,Item_Desc from usr_0_da_error_desc_Master with(nolock) where isactive=:isactive AND workstename=:workstename order by id ASC";
	params = "workstename=="+workstename+"~~isactive==Y";    //modified by shamily to get workstepname
